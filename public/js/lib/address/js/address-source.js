...
define(function(require, exports, module) {
    var Address = function(options) {
        this.settings = $.extend({}, Address.defaults, options);
        this.init();
    };
    ...
    Address.prototype = {
            ...
            create : function() {
                ...
                var $province = _this.settings.province,
                    $city = _this.settings.city,
                ...
                if($city.val() === ''){
                    ...
                    _this.changeAction($city, data, $province, $city.val(), cityHtml);
                }
                ...
                if(_this.settings.level !== 0){
                    if($area.val() === ''){
                        $area.html(areaHtml);
                    }else{
                        _this.changeAction($area, data, $city, $area.val(), areaHtml);
                    }

                    _this.settings.areaParent.show();

                    initArr = [$province,$city,$area];
                }else{
                    _this.settings.areaParent.hide();

                    initArr = [$province,$city];
                }
             ...
             * 获取相关层级的数据
             */
            changeAction : function ($elem, data, $parentObj, elemVal, elemHtml){
                var thisVal = $parentObj.val().split('&&')[0];

                var j = 0;

                var htmlStr = elemHtml;
            ...
            bindEvent : function() {
                ...
                _this.settings.province.on('change',function(){
                    var $this = $(this);

                    if($this.val() !== ''){
                        _this.changeAction(_this.settings.city, addressData, $this, _this.settings.city.val(), cityHtml);
                    }else{
                        $(_this.settings.cityDummy).val(cityStr);
                        _this.settings.city.html(cityHtml);
                    }

                    if(_this.settings.level !== 0){
                        $(_this.settings.areaDummy).val(areaStr);
                        _this.settings.area.html(areaHtml);
                    }
                });

                if(_this.settings.level !== 0){
                    _this.settings.city.on('change',function(){
                        var $this = $(this);

                        if($this.val() !== ''){
                            _this.changeAction(_this.settings.area, addressData, $this, _this.settings.area.val(), areaHtml);
                        }else{
                            $(_this.settings.areaDummy).val(areaStr);
                            _this.settings.area.html(areaHtml);
                        }
